...
Every skill in `engineering/` or `productivity/` (the **promoted** buckets) must have a reference in the top-level `README.md` and an entry in `.claude-plugin/plugin.json`'s `skills` array (the Claude Code plugin ships exactly the promoted set). `package.json` exposes those two buckets as Pi package skill roots; keep the Pi package scoped to promoted skills only. Skills in `misc/`, `in-progress/`, and `deprecated/` must not appear in either manifest or the top-level `README.md` reference list.
